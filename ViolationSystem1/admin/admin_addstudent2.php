...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($course == 7 || $course == 8 || $course == 11 || $course == 12) {
    $department = 1;
  } elseif ($course == 10 || $course == 14 || $course == 9) {
    $department = 2;
  } elseif ($course == 13) {
    $department = 3; // or any default value
  }

  // CHECK IF USN ALREADY EXISTS
  $checkUSN = mysqli_query($conn, "SELECT usn FROM college_tbl WHERE usn = '$usn' LIMIT 1");
  ...
  $sql = "INSERT INTO college_tbl (usn, lname, fname, mname, idstrandcourse, glevel, section, genid, birthdate, address,  iddepartment) 
            VALUES ('$usn', '$lname', '$fname', '$mname', '$course', '$glevel','$section','$gender', '$bdate', '$address', '$department')";
  ...
  if (mysqli_query($conn, $sql)) {
    ...
    header("Location: admin_college.php"); // redirect even if error
    exit();
  }
}
?>
      ...
      <section class="report-section">
        ...
        <div class="report-container">
          ...
          <div class="report-form">
            <h2 class="report-title">ADD College student Form</h2>

            <form action="admin_addstudent2.php" method="POST" enctype="multipart/form-data">
...
<!-- g level -->
              <div class="form-group">
                ...
                <select id="glevel" name="glevel" required>
                  ...
                  <option value="4" id="glevel4">4</option>
                </select>
              </div>
  ...
  <script>
    ...
    document.getElementById("course").addEventListener("change", function () {
      ...
      const glevel4 = document.querySelector("#glevel option[value='4']");

      if (course >= 11 && course <= 14) {
        // Hide option 4
        glevel4.style.display = "none";

        // If user previously selected 3, reset selection
        if (document.getElementById("glevel").value == "3") {
          document.getElementById("glevel").value = "";
        }

      } else {
        // Show option 4
        glevel4.style.display = "block";
      }
    });
